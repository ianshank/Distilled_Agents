...
def main():
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default="sample_training_data.jsonl",
        help="Input training data file",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./software_dev_agent",
    ...
    parser.add_argument(
        "--distillation_alpha", type=float, default=0.6, help="Weight for distillation loss"
    )
    parser.add_argument(
        "--temperature", type=float, default=2.5, help="Temperature for knowledge distillation"
    )
    parser.add_argument(
        "--use_wandb", action="store_true", help="Use Weights & Biases for tracking"
    )
    parser.add_argument(
        "--prepare_data_only", action="store_true", help="Only prepare dataset, skip training"
    )
    ...
    if num_examples < 10:
        logger.warning(
            f"Only {num_examples} examples available. Consider adding more training data."
        )
