...
def main() -> int:
    ...
    target_modules = (
        ...
        else resolve_target_modules_for_model(
            args.model_name_or_path, revision=getattr(args, "model_revision", None)
        )
    )
    ...
    # nosec: B615 - Local JSON file loading; Hub is not contacted (green-trunk-ci)
    raw_dataset = load_dataset("json", data_files=str(dataset_path), split="train")  # nosec: B615
