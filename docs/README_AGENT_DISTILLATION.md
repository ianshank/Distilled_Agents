...
| SDAR / AgentArk PAD | Deferred. SDPO with observations as `privileged_context` is the honest TRL mapping | Launcher hyperparams; gated OPSD on GRPO |
| TRL GKD / on-policy KD | Optional behind ADR 0008 (`trl==0.15.2`, `MANGOMAS_GKD_ENABLED`, alpha default 0.0, label-masked, vocab-checked) | Enabling by default before pass@k gates green; cross-vocab KL; SageMaker 4.26/1.13 |
...
Execution-consistent SAG; CodeAct subprocess sandbox; SageMaker `trajectory_mode`; SDAR / AgentArk PAD; live search; MCP autotools; `InputValidator` regex edits. (TRL GKD is implemented optionally behind ADR 0008).
