...
- [x] 8. Eval: verify solver is gated against `golden-passk-aqa` hard-slice Pass@K
  gate; solver MUST NOT be marked done without hard-slice gate. Close ONLY when E1 is green on dispose-bound fixtures (sqe_hard_ood solver-before-final_answer fixtures, not Echo/final_answer-only). COMPLETE: E1 green on dispose-bound fixtures (PR #34); not Echo/final_answer-only. Follow-on tracking in openspec/changes/sqe-dispose-e2e-bind/.
