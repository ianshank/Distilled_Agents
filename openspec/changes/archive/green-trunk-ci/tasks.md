...
- [x] 1. `ruff format` the files failing `ruff format --check` (incl. at least:
...
- [x] 2. Fix `enhanced_system/harness/data_governance.py` `redact_text` return
  type (`str(...)` / cast) so mypy `no-any-return` is clean.
- [x] 3. Pin `revision=` (or documented local-only path) for Hub
  `from_pretrained` in `scripts/training/distill/trainer.py`; justify
  `# nosec B615` only where Hub is not contacted (e.g. local JSON
  `load_dataset`).
- [x] 4. For AQA/security subprocess: keep `shell=False` list args; add
...
- [x] 5. Confirm `make validate` locally equivalent gates: lint, typecheck
  subset CI uses, bandit.
- [x] 6. CHANGELOG entry under Fixed.
- [x] 7. Merge only when Actions lint + types + security + test are green on
  the PR tip.
