...
- [ ] 2. Implement `SqeConstraintSolverTool` (primary id `sqe_constraint_solver`,
  pure Python DAG topo + boolean condition tree evaluation; deprecate or alias
  vague `ConstraintCheckTool` name) in `enhanced_system/harness/tools/registry.py`;
  register id in `TOOL_REGISTRY`.
...
- [ ] 4. Unit tests: valid DAG solve (lexicographically least topo order),
  ...
  with reject codes (`SCHEMA_VIOLATION`, `SYNTAX_INVALID`, `UNSUPPORTED_THEORY`,
