...
class AdaptiveRouter:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.routing_strategy = RoutingStrategy(config.get("routing_strategy", "cost_optimized"))

        # Load agent profiles
        self.agent_profiles: Dict[str, AgentProfile] = {}
        self._load_agent_profiles(config.get("agent_profiles_path"))

        logger.info(f"AdaptiveRouter initialized (strategy: {self.routing_strategy.value})")
        ...
        try:
            ...
            for profile_data in profiles_data.get("agents", []):
                profile = AgentProfile(
                    ...
                    max_complexity=TaskComplexity(profile_data.get("max_complexity", "complex")),
                )
    ...
    def route_task(
        ...
        # Generate reasoning
        reasoning = self._generate_reasoning(task_type, complexity, selected_agents)
    ...
    def _estimate_complexity_heuristic(self, task: str) -> TaskComplexity:
        ...
        high_indicators = sum(
            1 for indicator in self.COMPLEXITY_INDICATORS["high"] if indicator in task_lower
        )

        low_indicators = sum(
            1 for indicator in self.COMPLEXITY_INDICATORS["low"] if indicator in task_lower
        )
        ...
        complexity_score = high_indicators - low_indicators + length_score + multi_step * 0.5
    ...
    def _select_for_simple_task(
        self, task_type: str, constraints: Optional[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _generate_reasoning(
        self, task_type: str, complexity: TaskComplexity, agents: List[str]
    ) -> str:
        """Generate reasoning for routing decision"""
        agent_names = [
            self.agent_profiles[aid].name for aid in agents if aid in self.agent_profiles
        ]
