...
class ConfidenceCalibrator:
    ...
    def calibrate_confidence(
        ...
        factors = self._calculate_confidence_factors(agent, task_type, result or {})
    ...
    def _apply_calibration(self, raw_confidence: float, agent: str, task_type: str) -> float:
        """
    ...
    def _calculate_confidence_factors(
        self, agent: str, task_type: str, result: Dict[str, Any]
    ) -> Dict[str, float]:
        """
    ...
    def _get_agent_reliability(self, agent: str) -> float:
        ...
        accuracies = []
        for _task_type, data_points in self.calibration_data[agent].items():
            if data_points:
    ...
    def _adjust_for_factors(self, calibrated: float, factors: Dict[str, float]) -> float:
        ...
        factor_score = sum(factors.get(factor, 0.5) * weight for factor, weight in weights.items())
    ...
    def _generate_explanation(
        ...
        Returns:
            Explanation string
        """
        explanation_parts = [f"Confidence: {final_confidence:.2%} ({reliability_band.value})"]
    ...
    def update_calibration_data(
        self, agent: str, task_type: str, predicted_confidence: float, actual_accuracy: float
    ) -> None:
        """
        ...
        self.calibration_data[agent][task_type].append((predicted_confidence, actual_accuracy))
