...
class ConsensusInference:
    ...
    async def infer_with_consensus(
        ...
        **kwargs,
        ...
        if len(agent_funcs) < num_agents:
            logger.warning(f"Requested {num_agents} agents but only {len(agent_funcs)} available")
        ...
        confidence = self._calculate_overall_confidence(valid_responses, agreement_score)
    ...
    async def _execute_agent(
        self, agent_func: Callable, task: str, agent_id: str, **kwargs
    ) -> AgentResponse:
        """
    ...
    def _calculate_embedding_agreement(self, responses: List[AgentResponse]) -> float:
        """
    ...
    def _calculate_exact_match_agreement(self, responses: List[AgentResponse]) -> float:
        """
    ...
    def _confidence_weighted_ensemble(self, responses: List[AgentResponse]) -> str:
        """
        ...
        weighted_responses = [(r.response, r.confidence / total_confidence) for r in responses]
    ...
    def _calculate_overall_confidence(
        self, responses: List[AgentResponse], agreement_score: float
    ) -> float:
        """
...
async def run_consensus_inference(
    task: str, agents: List[Callable], config: Dict[str, Any], **kwargs
) -> ConsensusResult:
    """
