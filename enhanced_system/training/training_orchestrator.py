...
class TrainingOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"TrainingOrchestrator initialized (max_parallel: {self.max_parallel_jobs})")
    ...
    async def train_all(self) -> Dict[str, TrainingJob]:
        ...
        logger.info(
            f"Training complete: {len(completed_jobs)} succeeded, {len(failed_jobs)} failed"
        )

        return self.jobs

    def _get_ready_jobs(self, completed: Set[str], failed: Set[str]) -> List[str]:
        """
        ...
        for job_id, _job in self.jobs.items():
            # Skip if already processed or running
            if job_id in completed or job_id in failed or job_id in self.running_jobs:
                continue
    ...
    async def _train_agent(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Job {job_id} completed in {(job.end_time - job.start_time).total_seconds():.1f}s"
            )
