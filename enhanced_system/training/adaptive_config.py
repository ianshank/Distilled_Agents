...
class AdaptiveTrainingConfig:
    ...
    def calculate_lr(self, step: int, total_steps: int) -> float:
        ...
        if self.lr_schedule.strategy == "cosine_annealing":
            import math
            progress = (step - self.lr_schedule.warmup_steps) / max(1, total_steps - self.lr_schedule.warmup_steps)
            cosine_decay = 0.5 * (1 + math.cos(math.pi * progress))
            lr = self.lr_schedule.min_lr + (self.lr_schedule.max_lr - self.lr_schedule.min_lr) * cosine_decay
            return lr
        
        elif self.lr_schedule.strategy == "linear":
            progress = (step - self.lr_schedule.warmup_steps) / max(1, total_steps - self.lr_schedule.warmup_steps)
            lr = self.lr_schedule.max_lr - (self.lr_schedule.max_lr - self.lr_schedule.min_lr) * progress
    ...
    def log_config(self):
        ...
        logger.info(f"Early stopping: {self.early_stopping.enabled} (patience: {self.early_stopping.patience})")
        logger.info(f"Mixed precision: {self.mixed_precision.enabled} ({self.mixed_precision.precision})")
